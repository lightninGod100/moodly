...
const validateEmail = (req, res, next) => {
  try {
    ...
    // Check if email is provided - NO SERVER LOGGING (validation error)
    if (!rawEmail) {
    ...
    // Validate email format - NO SERVER LOGGING (validation error)
    ...
    // Check for plus sign (aliasing) - NO SERVER LOGGING (validation error)
    ...
    // Extract and validate domain - NO SERVER LOGGING (validation error)
    ...
    // Check against secure domain whitelist - NO SERVER LOGGING (validation error)
    ...
    const errorResponse = ErrorLogger.logAndCreateResponse(
      ...
      'EMAIL_VALIDATION_MIDDLEWARE',
      'email validation processing', // Custom context for middleware
      error,
      null // No user ID available in middleware
    );
